...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'], $_GET['id'])) {
    $id = (int) $_GET['id'];
    if ($_GET['action'] === 'edit') {
        $stmt = $conn->prepare('SELECT bk.id, bk.kriteria_id, bk.nilai_bobot FROM bobot_kriteria bk WHERE bk.id = ? LIMIT 1');
        $stmt->bind_param('i', $id);
        $stmt->execute();
        $editBobot = $stmt->get_result()->fetch_assoc();
    } elseif ($_GET['action'] === 'delete') {
        $stmt = $conn->prepare('DELETE FROM bobot_kriteria WHERE id = ?');
        $stmt->bind_param('i', $id);
        $stmt->execute();
        setFlash('success', 'Bobot berhasil dihapus.');
        redirect('bobot.php');
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $bobotId = (int) ($_POST['bobot_id'] ?? 0);
    $kriteriaId = (int) ($_POST['kriteria_id'] ?? 0);
    $nilaiInput = trim($_POST['nilai_bobot'] ?? '0');
    $nilai = (float) str_replace(',', '.', $nilaiInput);

    if ($kriteriaId > 0 && $nilai > 0) {
        if ($bobotId > 0) {
            $stmt = $conn->prepare('UPDATE bobot_kriteria SET kriteria_id = ?, nilai_bobot = ? WHERE id = ?');
            $stmt->bind_param('idi', $kriteriaId, $nilai, $bobotId);
            $stmt->execute();
            setFlash('success', 'Bobot berhasil diperbarui.');
        } else {
            $stmt = $conn->prepare('INSERT INTO bobot_kriteria (kriteria_id, nilai_bobot) VALUES (?, ?)');
            $stmt->bind_param('id', $kriteriaId, $nilai);
            $stmt->execute();
            setFlash('success', 'Bobot berhasil ditambahkan.');
        }
        redirect('bobot.php');
    }

    setFlash('error', 'Kriteria dan bobot harus valid.');
    redirect('bobot.php');
}

$kriteriaResult = $conn->query('SELECT * FROM kriteria ORDER BY id');
$kriteria = $kriteriaResult->fetch_all(MYSQLI_ASSOC);

$bobotResult = $conn->query('SELECT bk.id, k.nama_kriteria, bk.nilai_bobot FROM bobot_kriteria bk JOIN kriteria k ON k.id = bk.kriteria_id ORDER BY bk.id DESC');
$bobotRows = $bobotResult->fetch_all(MYSQLI_ASSOC);
$flash = getFlash();
?>
<!DOCTYPE html>
<html lang="id">
    ...
    <main class="main-content">
        <header class="flex justify-between items-center h-16 px-margin-page sticky top-0 z-50 w-full bg-surface/80 backdrop-blur-md border-b border-outline-variant dark:border-outline">
            <div class="flex items-center gap-4">
                <h2 class="font-headline-sm text-headline-sm font-semibold text-on-surface dark:text-on-surface">Input Bobot</h2>
                <div class="h-6 w-[1px] bg-outline-variant"></div>
                <span class="font-label-caps text-label-caps text-on-surface-variant uppercase tracking-widest">Nilai bobot kriteria</span>
            </div>
        ...
        <div class="page-content">
            ...
            <div class="grid grid-2">
                <div class="card">
                    <h3 style="margin-top:0;">Tambah Bobot Kriteria</h3>
                    <form method="post" data-validate>
                        <input type="hidden" name="bobot_id" value="<?php echo e($editBobot['id'] ?? ''); ?>">
                        <div>
                            <label for="kriteria_id">Kriteria</label>
                            <select id="kriteria_id" name="kriteria_id" required>
                                <option value="">Pilih kriteria</option>
                                <?php foreach ($kriteria as $item): ?>
                                    <option value="<?php echo (int) $item['id']; ?>" <?php echo isset($editBobot['kriteria_id']) && $editBobot['kriteria_id'] == $item['id'] ? 'selected' : ''; ?>><?php echo e($item['nama_kriteria']); ?> (<?php echo e($item['tipe']); ?>)</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div style="margin-top:16px;">
                            <label for="nilai_bobot">Nilai Bobot</label>
                            <input id="nilai_bobot" name="nilai_bobot" type="text" pattern="^[0-9]+([\.,][0-9]+)?$" placeholder="0,25" value="<?php echo e($editBobot['nilai_bobot'] ?? ''); ?>" required>
                        </div>
                        <div class="row" style="margin-top:16px;">
                            <button type="submit" class="btn btn-primary"><?php echo $editBobot ? 'Perbarui Bobot' : 'Simpan Bobot'; ?></button>
                            <?php if ($editBobot): ?>
                                <a class="btn btn-secondary" href="bobot.php">Batal</a>
                            <?php else: ?>
                ...
                <div class="card">
                    <h3 style="margin-top:0;">Daftar Bobot</h3>
                    <div class="table-wrap">
                        <table>
                            <thead><tr><th>No</th><th>Kriteria</th><th>Bobot</th><th>Aksi</th></tr></thead>
                            <tbody>
                                <?php if ($bobotRows): foreach ($bobotRows as $idx => $row): ?>
                                    <tr>
                                        <td><?php echo $idx + 1; ?></td>
                                        <td><?php echo e($row['nama_kriteria']); ?></td>
                                        <td><?php echo e($row['nilai_bobot']); ?></td>
                                        <td>
                                            <a class="btn btn-secondary" href="bobot.php?action=edit&id=<?php echo (int) $row['id']; ?>">Edit</a>
                                            <a class="btn btn-danger" href="bobot.php?action=delete&id=<?php echo (int) $row['id']; ?>" onclick="return confirm('Hapus bobot ini?');">Hapus</a>
                                        </td>
                                    </tr>
                                <?php endforeach; else: ?>
                                    <tr><td colspan="4" class="muted">Belum ada bobot.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
