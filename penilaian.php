...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'], $_GET['id'])) {
    $id = (int) $_GET['id'];
    if ($_GET['action'] === 'edit') {
        $stmt = $conn->prepare('SELECT * FROM penilaian_supplier WHERE id = ? LIMIT 1');
        $stmt->bind_param('i', $id);
        $stmt->execute();
        $editEvaluation = $stmt->get_result()->fetch_assoc();
        if ($editEvaluation) {
            $selectedSupplierId = $editEvaluation['supplier_id'];
        }
    } elseif ($_GET['action'] === 'delete') {
        $stmt = $conn->prepare('DELETE FROM penilaian_supplier WHERE id = ?');
        $stmt->bind_param('i', $id);
        $stmt->execute();
        setFlash('success', 'Penilaian supplier berhasil dihapus.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $nilaiInput = trim($_POST['nilai'] ?? '0');
    $nilai = (float) str_replace(',', '.', $nilaiInput);

    if ($supplierId > 0 && $kriteriaId > 0 && $nilai >= 0) {
        if ($evaluationId > 0) {
            $stmt = $conn->prepare('UPDATE penilaian_supplier SET supplier_id = ?, kriteria_id = ?, nilai = ? WHERE id = ?');
            $stmt->bind_param('iidi', $supplierId, $kriteriaId, $nilai, $evaluationId);
            $stmt->execute();
            setFlash('success', 'Penilaian supplier berhasil diperbarui.');
        } else {
            $stmt = $conn->prepare('INSERT INTO penilaian_supplier (supplier_id, kriteria_id, nilai) VALUES (?, ?, ?)');
            $stmt->bind_param('iid', $supplierId, $kriteriaId, $nilai);
            $stmt->execute();
            setFlash('success', 'Penilaian supplier berhasil disimpan.');
        }
        redirect('penilaian.php?supplier_id=' . $supplierId);
    }
    setFlash('error', 'Input penilaian tidak valid. Pastikan semua field terisi.');
    redirect('penilaian.php?supplier_id=' . $supplierId);
}

$supplierResult = $conn->query('SELECT * FROM supplier ORDER BY nama_supplier');
$supplierRows = $supplierResult->fetch_all(MYSQLI_ASSOC);
$kriteriaResult = $conn->query('SELECT * FROM kriteria ORDER BY nama_kriteria');
$kriteriaRows = $kriteriaResult->fetch_all(MYSQLI_ASSOC);
$evaluationResult = $conn->query('SELECT ps.id, s.nama_supplier, k.nama_kriteria, ps.nilai, ps.created_at, ps.supplier_id, ps.kriteria_id FROM penilaian_supplier ps JOIN supplier s ON s.id = ps.supplier_id JOIN kriteria k ON k.id = ps.kriteria_id ORDER BY ps.created_at DESC');
$evaluationRows = $evaluationResult->fetch_all(MYSQLI_ASSOC);
$flash = getFlash();
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Penilaian Supplier</title>
    ...
    <main class="main-content">
        <header class="flex justify-between items-center h-16 px-margin-page sticky top-0 z-50 w-full bg-surface/80 backdrop-blur-md border-b border-outline-variant dark:border-outline">
            <div class="flex items-center gap-4">
                <h2 class="font-headline-sm text-headline-sm font-semibold text-on-surface dark:text-on-surface">Penilaian Supplier</h2>
                <div class="h-6 w-[1px] bg-outline-variant"></div>
                <span class="font-label-caps text-label-caps text-on-surface-variant uppercase tracking-widest">Menilai supplier per kriteria</span>
            </div>
        ...
        <div class="page-content">
            ...
            <div class="grid grid-2">
                <div class="card">
                    <h3 style="margin-top:0;">Form Penilaian</h3>
                    <form method="post" data-validate>
                        <input type="hidden" name="evaluation_id" value="<?php echo e($editEvaluation['id'] ?? ''); ?>">
                        <div>
                            <label for="supplier_id">Supplier</label>
                            <select id="supplier_id" name="supplier_id" required>
                                <option value="">Pilih supplier</option>
                                <?php foreach ($supplierRows as $supplier): ?>
                                    <option value="<?php echo (int) $supplier['id']; ?>" <?php echo ((isset($editEvaluation['supplier_id']) && $editEvaluation['supplier_id'] == $supplier['id']) || ($selectedSupplierId > 0 && $selectedSupplierId == $supplier['id'])) ? 'selected' : ''; ?>><?php echo e($supplier['nama_supplier']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div style="margin-top:16px;">
                            ...
                            <select id="kriteria_id" name="kriteria_id" required>
                                ...
                                <?php foreach ($kriteriaRows as $kriteria): ?>
                                    <option value="<?php echo (int) $kriteria['id']; ?>" <?php echo isset($editEvaluation['kriteria_id']) && $editEvaluation['kriteria_id'] == $kriteria['id'] ? 'selected' : ''; ?>><?php echo e($kriteria['nama_kriteria']); ?> (<?php echo e($kriteria['tipe']); ?>)</option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div style="margin-top:16px;">
                            <label for="nilai">Nilai</label>
                            <input id="nilai" name="nilai" type="text" pattern="^[0-9]+([\.,][0-9]+)?$" placeholder="0,25" required value="<?php echo e($editEvaluation['nilai'] ?? ''); ?>">
                        </div>
                        <div class="row" style="margin-top:16px;">
                            <button type="submit" class="btn btn-primary"><?php echo $editEvaluation ? 'Perbarui Penilaian' : 'Simpan Penilaian'; ?></button>
                            <?php if ($editEvaluation): ?>
                                <a class="btn btn-secondary" href="penilaian.php">Batal</a>
                            <?php else: ?>
                ...
                <div class="card">
                    ...
                    <div class="table-wrap">
                        <table>
                            <thead><tr><th>No</th><th>Supplier</th><th>Kriteria</th><th>Nilai</th><th>Waktu</th><th>Aksi</th></tr></thead>
                            <tbody>
                                <?php if ($evaluationRows): foreach ($evaluationRows as $idx => $row): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-secondary" href="penilaian.php?action=edit&id=<?php echo (int) $row['id']; ?>">Edit</a>
                                            <a class="btn btn-danger" href="penilaian.php?action=delete&id=<?php echo (int) $row['id']; ?>" onclick="return confirm('Hapus penilaian ini?');">Hapus</a>
                                        </td>
                                    </tr>
                                <?php endforeach; else: ?>
                                    <tr><td colspan="5" class="muted">Belum ada penilaian.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
