...
$totalWeight = 0.0;
foreach ($kriteria as $krit) {
    $w = (float) str_replace(',', '.', $krit['nilai_bobot']);
    $weights[$krit['id']] = $w;
    $totalWeight += $w;
}
...
foreach ($suppliers as $supplierId => $kriteriaValues) {
    foreach ($kriteria as $krit) {
        ...
        $normalized[$supplierId][$krit['id']] = $tipe === 'cost'
...
foreach ($suppliers as $supplierId => $kriteriaValues) {
    $total = 0.0;
    foreach ($kriteria as $krit) {
        if (!isset($normalized[$supplierId][$krit['id']]) || !isset($weights[$krit['id']])) {
            continue;
        }
        $weight = $weights[$krit['id']];
        $rij = $normalized[$supplierId][$krit['id']];
        $total += $weight * $rij;
    }
    $preference[$supplierId] = $total;
}
    ...
    <main class="main-content">
        <header class="flex justify-between items-center h-16 px-margin-page sticky top-0 z-50 w-full bg-surface/80 backdrop-blur-md border-b border-outline-variant dark:border-outline">
            <div class="flex items-center gap-4">
                <h2 class="font-headline-sm text-headline-sm font-semibold text-on-surface dark:text-on-surface">Preferensi</h2>
                <div class="h-6 w-[1px] bg-outline-variant"></div>
                <span class="font-label-caps text-label-caps text-on-surface-variant uppercase tracking-widest">Nilai total V</span>
            </div>
        ...
        <div class="page-content">
            ...
            <div class="card">
                <h3 style="margin-top:0;">Nilai Preferensi Supplier</h3>
                <div class="table-wrap">
                    <table>
                        <thead>
                            <tr><th>Supplier</th><th>Nilai Preferensi (V)</th><th>Status</th></tr>
                        </thead>
                        <tbody>
                            <?php if ($ranking): foreach ($ranking as $supplierId => $score): ?>
                                <tr>
                            ...
                            <?php endforeach; else: ?>
                                <tr><td colspan="3" class="muted">Belum ada data kriteria, bobot, atau penilaian untuk menghitung preferensi.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="card" style="margin-top:16px;">
                <h3 style="margin-top:0;">Detail Kriteria dan Bobot</h3>
                <div class="table-wrap">
                    <table>
                        <thead><tr><th>Kriteria</th><th>Tipe</th><th>Bobot</th></tr></thead>
                        <tbody>
                            <?php if ($kriteria): foreach ($kriteria as $krit): ?>
                                <tr>
                                    <td><?php echo e($krit['nama_kriteria']); ?></td>
                                    <td><?php echo e($krit['tipe']); ?></td>
                                    <td><?php echo e($krit['nilai_bobot']); ?></td>
                                </tr>
                            <?php endforeach; else: ?>
                                <tr><td colspan="3" class="muted">Belum ada bobot kriteria.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
