...
foreach ($penilaianRows as $row) {
    $valuesByKriteria[$row['kriteria_id']][] = $row['nilai'];
    $suppliers[$row['supplier_id']][$row['kriteria_id']] = $row['nilai'];
}
...
foreach ($suppliers as $supplierId => $kriteriaValues) {
    ...
    foreach ($kriteria as $krit) {
        ...
        $nilai = $kriteriaValues[$krit['id']];
        $normalized = $krit['tipe'] === 'cost'
            ? ($nilai > 0 ? $summary[$krit['id']]['min'] / $nilai : 0)
            : ($summary[$krit['id']]['max'] > 0 ? $nilai / $summary[$krit['id']]['max'] : 0);
        $score += $krit['nilai_bobot'] * $normalized;
    }
...
<body>
<div class="app-shell">
    <aside class="w-nav-width h-screen sticky top-0 left-0 bg-white text-black border-r border-outline-variant flex flex-col h-full py-stack-lg px-stack-md shrink-0 no-print">
    ...
    <main class="main-content">
        <header class="flex justify-between items-center h-16 px-margin-page sticky top-0 z-50 w-full bg-surface/80 backdrop-blur-md border-b border-outline-variant dark:border-outline no-print">
            <div class="flex items-center gap-4">
                <h2 class="font-headline-sm text-headline-sm font-semibold text-on-surface dark:text-on-surface">Laporan</h2>
                <div class="h-6 w-[1px] bg-outline-variant"></div>
                <span class="font-label-caps text-label-caps text-on-surface-variant uppercase tracking-widest">Ringkasan ranking supplier</span>
            </div>
        ...
        <div class="page-content">
            ...
            <div class="card">
                <h3 style="margin-top:0;">Ringkasan Ranking Supplier</h3>
                <div class="table-wrap">
                    <table>
                        <thead><tr><th>Rank</th><th>Supplier</th><th>Skor Preferensi</th><th>Status</th></tr></thead>
                        <tbody>
                            <?php if ($ranking): $rank = 1; foreach ($ranking as $supplierId => $score): ?>
                                <tr>
                                    <td><?php echo $rank++; ?></td>
                                    <td><?php echo e($supplierNames[$supplierId] ?? 'Supplier #'.$supplierId); ?></td>
                                    <td><?php echo e(number_format($score, 4, ',', '')); ?></td>
                                    <td><?php echo e($score >= 0.8 ? 'Direkomendasikan' : 'Perlu Evaluasi'); ?></td>
                                </tr>
                            <?php endforeach; else: ?>
                                <tr><td colspan="4" class="muted">Data belum tersedia untuk laporan.</td></tr>
                            <?php endif; ?>
                        </tbody></table>
                </div>
            </div>
            <div class="card" style="margin-top:16px;">
                ...
                <p class="muted">Laporan ini bersumber dari data supplier, kriteria, bobot, dan penilaian yang tersimpan di sistem. Tombol Export PDF membuka dialog cetak browser.</p>
            </div>
            <div class="card print-only" style="margin-top:16px;">
                <h3>Laporan Cetak</h3>
