...
foreach ($kriteria as $krit) {
    $kritId = $krit['id'];
    if (empty($valuesByKriteria[$kritId])) {
        continue;
    }
    $max = max($valuesByKriteria[$kritId]);
    $min = min($valuesByKriteria[$kritId]);
    $summary[$kritId] = ['max' => $max, 'min' => $min];
}

foreach ($suppliers as $supplierId => $kriteriaValues) {
    foreach ($kriteria as $krit) {
        $kritId = $krit['id'];
        if (!isset($kriteriaValues[$kritId]) || !isset($summary[$kritId])) {
            $normalisasi[$supplierId][$kritId] = null;
            continue;
        }
        $nilai = $kriteriaValues[$kritId];
        $tipe = strtolower(trim($krit['tipe']));
        if ($tipe === 'cost') {
            $normalisasi[$supplierId][$kritId] = $nilai > 0 ? $summary[$kritId]['min'] / $nilai : 0;
        } else {
            $normalisasi[$supplierId][$kritId] = $summary[$kritId]['max'] > 0 ? $nilai / $summary[$kritId]['max'] : 0;
        }
    }
}
    ...
    <main class="main-content">
        <header class="flex justify-between items-center h-16 px-margin-page sticky top-0 z-50 w-full bg-surface/80 backdrop-blur-md border-b border-outline-variant dark:border-outline">
            <div class="flex items-center gap-4">
                <h2 class="font-headline-sm text-headline-sm font-semibold text-on-surface dark:text-on-surface">Normalisasi</h2>
                <div class="h-6 w-[1px] bg-outline-variant"></div>
                <span class="font-label-caps text-label-caps text-on-surface-variant uppercase tracking-widest">Rumus otomatis SAW</span>
            </div>
        ...
        <div class="page-content">
            ...
            <div class="card" style="background:#f7f9ff;">
                <h3 style="margin-top:0;">Rumus Normalisasi</h3>
                <p class="muted">Gunakan rumus SAW sesuai tipe kriteria:</p>
                <ul>
                    <li><strong>Benefit:</strong> R<sub>ij</sub> = nilai<sub>ij</sub> / max(kriteria<sub>j</sub>)</li>
                    <li><strong>Cost:</strong> R<sub>ij</sub> = min(kriteria<sub>j</sub>) / nilai<sub>ij</sub></li>
                </ul>
            </div>
            <div class="card">
                <h3 style="margin-top:0;">Matriks Normalisasi</h3>
                        ...
                        <thead>
                            <tr>
                                ...
                                <?php foreach ($kriteria as $krit): ?>
                                    <th><?php echo e($krit['nama_kriteria']); ?> <span class="small">(<?php echo e($krit['tipe']); ?>)</span></th>
                                <?php endforeach; ?>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if ($suppliers): foreach ($suppliers as $supplierId => $values): ?>
                                <tr>
                                    <td><?php echo e($supplierNames[$supplierId] ?? 'Supplier #'.$supplierId); ?></td>
                                    <?php foreach ($kriteria as $krit): ?>
                                        <td><?php echo is_null($normalisasi[$supplierId][$krit['id']]) ? '-' : e(number_format($normalisasi[$supplierId][$krit['id']], 4, ',', '')); ?></td>
                                    <?php endforeach; ?>
                                </tr>
                            <?php endforeach; else: ?>
                                <tr><td colspan="<?php echo count($kriteria) + 1; ?>" class="muted">Belum ada penilaian supplier untuk dinormalisasi.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="card" style="margin-top:16px;">
                <h3 style="margin-top:0;">Ringkasan Kriteria</h3>
                <div class="grid grid-2">
                    <?php foreach ($kriteria as $krit): ?>
                        <div class="card" style="background:#f4f7ff;">
                            <div class="stat-label"><?php echo e($krit['nama_kriteria']); ?> (<?php echo e($krit['tipe']); ?>)</div>
                            <div class="stat-value"><?php echo isset($summary[$krit['id']]) ? e($summary[$krit['id']]['max']) . ' / ' . e($summary[$krit['id']]['min']) : '-'; ?></div>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
